...
- 문자열 기반 grounding은 인용의 실제 존재와 필수 anchor를 검증하지만, 모든 의미적 관련성을
  완전히 판정하지는 않는다.
- 예제 6건은 실행 재현용이며 실제 DART 전체 성능을 대표하지 않는다.
- DART HTML과 질문은 live provider 실행 시 외부 서비스로 전송될 수 있다. 전송 권한이 있는
  데이터만 사용한다.
- `.env`, API 키, 실제 DART HTML, 생성된 `reports/`는 Git에 추가하지 않는다.
- 기존 실행 디렉터리를 덮어쓰거나 재개하지 않는다. 부분 실행을 보존하고 새 run ID를 사용한다.
