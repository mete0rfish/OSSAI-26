...
- 점수 채점 방식에 문제가 없는지 (AI Agent 관련 지식 부족)
- 현재 HTML에서 원하는 값을 응답하는 방식에 더해, 해당 응답을 파싱하는 코드를 생성하도록 확장할 계획인데 어떤 방식으로 더해나가야할지
- 간단한 질문은 대체로 답을 구했지만, 공시 데이터가 법인마다 작성형식이 달라 이를 이해시키는데 한계 존재
